...
import { 
  Leaf, ShoppingCart, Users, Star, ArrowRight,
  Flame,
} from 'lucide-react';
...
export default function HomePage() {
  ...
  // ============================================
  // FETCH ALL DATA (dengan anti-spam)
  // ============================================
  const fetchAllData = useCallback(async () => {
    // Guard 1: Already fetching
    if (isFetchingRef.current) {
      if (process.env.NODE_ENV === 'development') console.log('⏸️ [HOME] Skip - already fetching');
      return;
    }

    // Guard 2: Already fetched
    if (hasFetchedRef.current) {
      if (process.env.NODE_ENV === 'development') console.log('️ [HOME] Skip - already fetched');
      return;
    }
    ...
    try {
      ...
      const [productsRes, reviewsRes, statsRes] = await Promise.allSettled([
        fetch('/api/products?sort=sold_count&order=desc&limit=8', { headers })
          .then(res => res.json()),
        fetch('/api/reviews/top-rated?limit=5', { headers })
          .then(res => res.json()),
        fetch('/api/statistics/dashboard', { headers })
          .then(res => res.json()),
      ]);
      ...
      if (productsRes.status === 'fulfilled' && productsRes.value?.success) {
        const filtered = (productsRes.value.products || []).filter((p: Product) => 
          p.status === 'ready_stock' || p.status === 'pre_order'
        );
      ...
      if (reviewsRes.status === 'fulfilled' && reviewsRes.value?.success) {
        setReviews(reviewsRes.value.data?.reviews || reviewsRes.value.reviews || []);
      }
  ...
  return (
    <div className="animate-fade-in min-h-screen pb-20" data-theme={resolvedTheme}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8">
        {/* Hero Section */}
        <section className="bg-linear-to-br from-primary to-secondary rounded-3xl p-8 text-white relative overflow-hidden">
        ...
        {/* About Agri X */}
        <section className="bg-surface rounded-3xl p-6">
          ...
          <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-2">
            {reviews.length > 0 ? (
              reviews.map((review) => (
                <div
                  key={review.id}
                  className="shrink-0 bg-surface rounded-2xl p-6 min-w-70 max-w-80"
                >
        ...
        {/* CTA Section */}
        <section className="bg-linear-to-br from-primary to-secondary rounded-3xl p-8 text-center text-white">
