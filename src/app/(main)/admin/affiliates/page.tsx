...
import {
  ...
  DollarSign, ShoppingCart, Filter, Menu, MousePointer,
} from 'lucide-react';
...
// ✅ Generate social media URL from platform + username
const generateSocialMediaUrl = (platform: string, username: string): string => {
  ...
  const platforms: Record<string, string> = {
    ...
    youtube: `https://youtube.com/${cleanUsername.startsWith('@') ? cleanUsername : '@' + cleanUsername}`,
...
// ✅ Normalize sosmed account data (handle various formats)
const normalizeSosmedAccount = (account: any): SosmedAccount => {
  const platform = account.platform?.toLowerCase() || 'instagram';
  const username = account.username || '';
  const url = account.url || account.link || generateSocialMediaUrl(platform, username);
...
// ✅ Parse sosmed_accounts (handle JSON string or array)
const parseSosmedAccounts = (data: any): SosmedAccount[] => {
  try {
    if (!data) return [];
    ...
    return parsed.map(normalizeSosmedAccount);
  } catch (error) {
...
export default function AdminAffiliatesPage() {
  ...
  // ✅ CRITICAL: Refs untuk cegah infinite loop
  ...
  // ============================================================================
  // ✅ STABLE FETCH FUNCTIONS (pakai useCallback + refs)
  // ============================================================================
  const fetchAffiliates = useCallback(async (force: boolean = false) => {
    // ✅ Guard 1: Already fetching
    if (isFetchingRef.current) {
      return;
    }

    // ✅ Guard 2: Throttle (kecuali force)
    const now = Date.now();
    ...
    try {
      ...
      const parsedApps = (result.data?.applications || []).map((app: any) => ({
        ...app,
        sosmedAccounts: parseSosmedAccounts(app.sosmedAccounts),
      }));
  ...
  }, [isAuthenticated, user, fetchAffiliates, fetchStats]);

  // ============================================================================
  // ✅ REFRESH DATA (saat filter/page berubah)
  // ============================================================================
  useEffect(() => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/affiliates/${selectedApp.id}/approve`, {
        method: 'POST',
        headers: {
      ...
      toast.success(`✅ Aplikasi ${selectedApp.user.name} disetujui! Kode: ${result.data.referralCode}`);
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/affiliates/${selectedApp.id}/reject`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const copyReferralCode = (code: string) => {
    if (!code) {
      toast.error('Kode referral belum tersedia');
      return;
    }
              ...
              <div className="card bg-gradient-to-br from-blue-500 to-blue-600 text-white">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold mt-1">{formatCompactNumber(stats.totalClicks)}</p>
                  </div>
                  ...
                  <table className="w-full">
                    <thead className="bg-surface border-b border-border">
                      <tr>
                        ...
                        <th className="text-left py-3 px-4 text-sm font-semibold text-text-secondary">Performa</th>
                        ...
                        <tr key={app.id} className="hover:bg-surface/50 transition-colors">
                          ...
                          <td className="py-4 px-4">
                            {app.referralCode ? (
                              ...
                              <span className="text-xs text-text-muted italic">Belum ada</span>
                            )}
                          </td>
                          <td className="py-4 px-4">
                            <div className="flex items-center gap-1">
                              {app.sosmedAccounts.slice(0, 4).map((sosmed, idx) => {
                                const Icon = PLATFORM_ICONS[sosmed.platform] || FaGlobe;
                                const color = PLATFORM_COLORS[sosmed.platform] || 'text-text-secondary';
                                const fullUrl = sosmed.url || '#';
                                ...
                                return (
                                  <a
                                    ...
                                    className="w-8 h-8 rounded-lg flex items-center justify-center bg-surface hover:bg-primary/10 hover:scale-110 transition-all"
                                    title={`${PLATFORM_LABELS[sosmed.platform] || sosmed.platform}: @${sosmed.username}`}
                            ...
                            {app.status === 'approved' ? (
                              <div className="text-xs space-y-1">
                                <p className="text-text-primary">
                                  <span className="font-semibold">{formatCompactNumber(app.stats.totalOrders)}</span>
                                  <span className="text-text-secondary"> orders</span>
                                </p>
                                <p className="text-green-600 font-semibold">
                                  {formatCurrency(app.stats.totalCommission)}
                                </p>
                              </div>
                            ) : (
                ...
                <div className="md:hidden space-y-3 p-4">
                  {applications.map((app) => (
                    <div key={app.id} className="bg-surface rounded-xl p-4 border border-border">
                        ...
                        {app.sosmedAccounts.slice(0, 4).map((sosmed, idx) => {
                          ...
                          return (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="w-8 h-8 rounded-lg flex items-center justify-center bg-background hover:bg-primary/10 transition-colors"
                              title={PLATFORM_LABELS[sosmed.platform] || sosmed.platform}
                              onClick={(e) => e.stopPropagation()}
                            >
                              <Icon className={`w-4 h-4 ${color}`} />
                            </a>
                      ...
                      {app.status === 'approved' && (
                        <div className="mb-3 p-2 bg-background rounded-lg text-xs">
                          <div className="flex justify-between">
                            <span className="text-text-secondary">Orders:</span>
                            <span className="font-semibold">{app.stats.totalOrders}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-text-secondary">Komisi:</span>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-background rounded-2xl w-full max-w-200 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              {/* ✅ Social Media Accounts - CLICKABLE LINKS */}
              <div className="card bg-surface/50">
                <h3 className="font-semibold text-text-primary mb-4 flex items-center gap-2">
                ...
                {selectedApp.sosmedAccounts.length > 0 ? (
                  <div className="space-y-3">
                    {selectedApp.sosmedAccounts.map((sosmed, idx) => {
                      const Icon = PLATFORM_ICONS[sosmed.platform] || FaGlobe;
                      const color = PLATFORM_COLORS[sosmed.platform] || 'text-text-secondary';
                      const label = PLATFORM_LABELS[sosmed.platform] || sosmed.platform;
                      const fullUrl = sosmed.url || '#';

                      return (
                        <div key={idx} className="flex items-center gap-3 p-3 bg-background rounded-xl border border-border hover:border-primary/30 transition-colors">
                          ...
                          {sosmed.followers !== undefined && (
                            <div className="text-right flex-shrink-0">
                              ...
                              <p className="font-semibold text-text-primary text-sm">
                                {formatCompactNumber(sosmed.followers)}
                              </p>
                            </div>
                          )}
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="p-2 hover:bg-primary/10 rounded-lg transition-colors text-primary flex-shrink-0"
                            title={`Buka ${label} profile`}
                            onClick={(e) => e.stopPropagation()}
                          >
                ...
                <div className="card bg-surface/50">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="text-center p-3 bg-background rounded-xl">
                      ...
                      <p className="text-xl font-bold text-text-primary">{selectedApp.stats.totalOrders}</p>
                      <p className="text-xs text-text-secondary mt-1">Pesanan</p>
                    </div>
                    <div className="text-center p-3 bg-background rounded-xl">
                      <MousePointer className="w-6 h-6 text-blue-500 mx-auto mb-1" />
                      <p className="text-xl font-bold text-text-primary">{formatCompactNumber(selectedApp.stats.totalClicks)}</p>
                      <p className="text-xs text-text-secondary mt-1">Klik</p>
                    </div>
                    <div className="text-center p-3 bg-background rounded-xl">
                      <FaMoneyBillWave className="w-6 h-6 text-purple-500 mx-auto mb-1" />
                      <p className="text-sm font-bold text-primary">{formatCurrency(selectedApp.stats.totalRevenue)}</p>
                      <p className="text-xs text-text-secondary mt-1">Revenue</p>
                    </div>
                    <div className="text-center p-3 bg-background rounded-xl">
                      <DollarSign className="w-6 h-6 text-green-500 mx-auto mb-1" />
                      <p className="text-sm font-bold text-green-500">{formatCurrency(selectedApp.stats.totalCommission)}</p>
          ...
          <div className="bg-background rounded-2xl w-full max-w-200">
            <div className="p-6">
              ...
              <p className="text-text-secondary text-center mb-4">
                {actionType === 'approve' ? (
                  <>Aplikasi affiliate dari <strong>{selectedApp.user.name}</strong> akan disetujui dan kode referral akan diaktifkan.</>
                ) : (
