  ...
  const sendVerification = async (index: number) => {
    try {
      ...
      const res = await fetch(
        '/api/affiliate/apply/request-verification',
        {
          method: 'POST',
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Loader2 className="w-full h-10 text-primary animate-spin" />
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background py-8 px-4">
      <div className="max-w-full mx-auto">
        ...
        <div className="text-center mb-8">
          <div className="w-full h-16 bg-gradient-to-br from-primary to-secondary rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Award className="w-full h-8 text-white" />
          </div>
          <h1 className="text-3xl font-bold text-text-primary">
            Daftar Affiliate
          </h1>
          <p className="text-text-secondary mt-2">
            Bergabunglah dengan program affiliate kami dan dapatkan komisi dari
            setiap penjualan
          </p>
        </div>
        ...
        <div className="card mb-6">
          ...
          <p className="text-xs text-text-secondary mt-2">
            Email ini akan digunakan untuk proses verifikasi akun affiliate.
          </p>
        </div>
        ...
        <div className="card bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800 mb-6">
          <div className="flex items-start gap-3">
            <AlertCircle className="max-w-5xl h-5 text-blue-500 flex-shrink-0 mt-0.5" />
            <div className="text-sm text-blue-700 dark:text-blue-300">
              <p className="font-semibold mb-1">Syarat & Ketentuan:</p>
              <ul className="list-disc list-inside space-y-2 text-sm">
                <li>Tambahkan minimal satu akun sosial media aktif.</li>
                <li>Setiap akun wajib diverifikasi melalui email.</li>
                <li>
                  Setelah seluruh akun diverifikasi, Anda dapat mengirim
                  pendaftaran.
                </li>
                <li>
                  Tim Agri-X akan melakukan peninjauan sebelum akun affiliate
                  diaktifkan.
                </li>
              </ul>
            </div>
        ...
        <form onSubmit={handleSubmit} className="card space-y-6">
          <div>
            <h2 className="text-xl font-bold text-text-primary mb-4 flex items-center gap-2">
              <LinkIcon className="w-ful h-5 text-primary" />
              Akun Sosial Media
            </h2>
                          ...
                          <div className="relative">
                            ...
                            {validation && (
                              <div className="absolute right-3 top-1/2 -translate-y-1/2">
                                {validation.valid ? (
                                  <CheckCircle className="w-full h-5 text-green-500" />
                                ) : (
                                  <AlertCircle className="w-full h-5 text-red-500" />
                                )}
                              </div>
                            )}
                          </div>
                        ...
                        {previewUrl && validation?.valid && (
                          <div className="bg-background rounded-lg p-3 border border-border">
                            <p className="text-xs text-text-secondary mb-1 flex items-center gap-1">
                              <ExternalLink className="max-w-5xl h-3" />
                              Preview Link:
                            </p>
                            <a
                              href={previewUrl}
                            ...
                            >
                              {previewUrl}
                              <ExternalLink className="max-w-5xl h-3 flex-shrink-0" />
                            </a>
                          </div>
                        )}
                        {validation?.valid &&
