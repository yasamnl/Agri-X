...
export default function ForumPage() {
  // ✅ FIX 2: Gunakan useRouter langsung dari next/navigation
  const router = useRouter();
  const { isAuthenticated } = useAuth();
  
  // ✅ FIX 3: Tambahkan state untuk client-side check
  ...
  // ✅ FIX 4: Set isClient setelah mount
  useEffect(() => {
    setIsClient(true);
  }, []);
      ...
      return (
        <div key={comment.id} className={`${depth > 0 ? 'mt-4' : 'mb-6 last:mb-0'}`}>
          <div className={`flex gap-3 ${depth > 0 ? 'ml-6 sm:ml-8 border-l-2 border-border pl-4' : ''}`}>
            <div className="w-8 h-8 rounded-full bg-linear-to-br from-primary/20 to-secondary/20 flex items-center justify-center text-xs font-bold shrink-0 overflow-hidden border border-border">
              {comment.user_avatar ? (
            ...
            <div className="flex-1">
              <div className="bg-surface p-3 rounded-2xl rounded-tl-none shadow-sm relative">
                ...
                <p className="text-sm text-text-secondary leading-relaxed wrap-break-word">{comment.content}</p>
  ...
  // ✅ FIX 5: Tambahkan loading state untuk client-side
  if (!isClient) {
    return (
    ...
    <div className="animate-fade-in min-h-screen bg-background pb-20 sm:pb-0 relative">
      ...
      {selectedPostId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-4 bg-black/60 backdrop-blur-sm animate-fade-in" onClick={(e) => e.target === e.currentTarget && closeCommentModal()}>
          <div ref={modalRef} className="bg-background w-full max-w-500 h-[80vh] sm:h-150 sm:w-125 rounded-2xl flex flex-col shadow-2xl animate-scale-up border border-border overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-3 border-t border-border bg-background flex items-center gap-3 shrink-0 relative z-10">
               {replyingTo && (
