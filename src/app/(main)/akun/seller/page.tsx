// src/app/(main)/akun/seller/page.tsx
'use client';

import { useState, useEffect, useCallback, useRef } from 'react';
import { useRouter } from 'next/navigation';
import {
  Store, MapPin, MessageSquare, Star, Settings, LogOut,
  Package, Camera, Loader2, ChevronRight, Bell, Shield,
  HelpCircle, UserCircle, BarChart3, ShoppingBag, Calendar,
} from 'lucide-react';
...
export default function SellerAccountPage() {
  ...
  // ✅ FIX: useCallback dengan proper dependencies
  const fetchUserProfile = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        hasFetchedProfileRef.current = true; // ✅ Mark as fetched
      }
    } catch (error: any) {
      console.error('❌ Fetch profile error:', error);
      // ✅ Jangan throw, biarkan UI handle error
  ...
  // ✅ FIX: useEffect dengan guard yang benar
  useEffect(() => {
  ...
  // ✅ FIX: Hanya fetch sekali saat authenticated
  useEffect(() => {
  ...
  const settingsItems = [
    {
      ...
      items: [
        ...
        { icon: Package, label: 'Produk Saya', action: 'products', href: '/seller/products' },
    ...
    <div className="min-h-screen bg-background pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <>
          ...
          {activeTab === 'orders' && <OrdersTab userId={user?.id} isSeller={true} />}
