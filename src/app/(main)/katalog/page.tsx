...
import { Search, X, SlidersHorizontal, Check, Loader2, Package } from 'lucide-react';
...
export default function KatalogPage() {
  // ✅ HAPUS useAuth() - katalog adalah halaman publik
  ...
  // ============================================
  useEffect(() => {
    // Reset pagination & products saat filter berubah
    setPage(1);
    setProducts([]);
    setHasMore(true);
    
    // Fetch dengan delay kecil untuk pastikan state reset
  ...
  const hasActiveFilters = selectedCategoryId !== null || selectedSortId !== 'all' || searchQuery !== '';
    ...
    <div className="animate-fade-in min-h-screen pb-20">
      ...
      {totalProducts > 0 && !isLoading && (
        <div className="mb-4 text-sm text-text-secondary">
          Menampilkan <span className="font-semibold text-text-primary">{products.length}</span> dari{' '}
          <span className="font-semibold text-text-primary">{totalProducts}</span> produk
        </div>
      )}
