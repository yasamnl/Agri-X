...
export async function GET(req: NextRequest) {
  try {
    ...
    // ✅ Fetch semua stats dalam satu query untuk performa optimal
    const [stats] = await pool.query(
      `SELECT 
        ...
        -- Pesanan aktif (pending/processing/shipped)
        (SELECT COUNT(DISTINCT o.id)
         FROM orders o
         INNER JOIN order_items oi ON o.id = oi.order_id
         INNER JOIN products p ON oi.product_id = p.id
         WHERE p.seller_id = ? 
         AND o.status IN ('pending', 'processing', 'shipped')) as active_orders,
         
        -- Rating rata-rata produk seller
