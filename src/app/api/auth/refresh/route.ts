...
export async function POST(req: NextRequest) {
  try {
    ...
    const sessionsResult: any = await pool.execute(
    ...
    // Generate token baru
    const userResult: any = await pool.execute('SELECT role FROM users WHERE id = ?', [userId]);
    const userRole = userResult[0]?.[0]?.role;
    if (!userRole) {
