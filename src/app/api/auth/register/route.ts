...
export async function POST(req: NextRequest) {
  try {
    ...
    //  FIX 1: pool.execute returns a tuple; extract the first element and cast to any to access .rows
    const existingResult = (await pool.execute(
      'SELECT id FROM users WHERE email = ?',
      [email]
    )) as any;

    // existingResult[0] is the QueryResult when using this driver
    const existingRows = existingResult[0]?.rows ?? existingResult.rows;

    if (existingRows.length > 0) {
      if (process.env.NODE_ENV === 'development') console.log(`[REGISTER] Email already exists: ${email}`);
      return NextResponse.json(
    ...
    const insertRows = insertResult[0]?.rows ?? insertResult.rows;
