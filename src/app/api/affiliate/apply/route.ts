  ...
  try {
    ...
    try {
      ...
      for (const account of sosmedAccounts) {
        ...
        // ✅ Ikuti status verified dari sosmed_accounts (JSON) yang dikirim frontend
        const isVerified = account.verified === true ? 1 : 0;

        await connection.query(
          `INSERT INTO affiliate_social_media 
           (affiliate_application_id, platform, username, link, verified, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, NOW(), NOW())`,
          [applicationId, account.platform, account.username, fullUrl, isVerified]
        );
      }
