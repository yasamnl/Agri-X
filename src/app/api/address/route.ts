...
export async function GET(req: NextRequest) {
  try {
    ...
    if (addresses.length > 0) {
      if (process.env.NODE_ENV === 'development') console.log('📬 [ADDRESS API] Returning addresses:', {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === 'development') console.log('📬 [ADDRESS API] Create request:', body);
    ...
    if (process.env.NODE_ENV === 'development') console.log('✅ [ADDRESS API] Address created:', insertedId);
