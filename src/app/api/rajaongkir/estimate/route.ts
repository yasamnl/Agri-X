...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('🚚 [SHIPPING API] Calling external API...');
    ...
    console.log('📦 [SHIPPING API] Raw Response Structure:', {
    ...
    if (couriers.length > 0) {
      console.log('🔍 [SHIPPING API] First courier sample:', JSON.stringify(couriers[0], null, 2));
    }
    ...
    const formattedData = couriers.map((item: any, index: number) => {
      ...
      if (index === 0) {
        console.log('💰 [SHIPPING API] Price analysis:', {
          rawPrice,
      ...
      // ✅ Handle NaN
      if (isNaN(price)) {
        console.warn('⚠️ [SHIPPING API] Invalid price, using 0:', rawPrice);
        price = 0;
      }
      ...
      if (price > 1000000) {
        console.warn('⚠️ [SHIPPING API] Abnormal price detected:', price, '→ Normalizing...');
        ...
        if (normalizedPrice >= 5000 && normalizedPrice <= 500000) {
          console.log('✅ [SHIPPING API] Price normalized:', price, '→', normalizedPrice);
          price = normalizedPrice;
        } else {
          console.warn('⚠️ [SHIPPING API] Normalized price still abnormal:', normalizedPrice);
    ...
    console.log('✅ [SHIPPING API] Formatted data:', formattedData);
