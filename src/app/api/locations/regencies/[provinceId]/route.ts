...
export async function GET(request: NextRequest, { params }: Params) {
  try {
    ...
    if (process.env.NODE_ENV === 'development') console.log('🔍 Fetching regencies for provinceId:', provinceId);
    ...
    if (process.env.NODE_ENV === 'development') console.log('✅ Query successful. Found', Array.isArray(rows) ? rows.length : 0, 'regencies');
