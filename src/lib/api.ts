...
export const productAPI = {
  ...
  getById: (id: number) => {
    return api.get<{
      error: string; success: boolean; product: any 
}>(`/api/products/${id}`);
  },
...
export const reviewAPI = {
  ...
  // Create review
  create: (p0: { productId: number | undefined; rating: number; comment: string; }, token: string | null, data: { productId: number; rating: number; comment?: string; orderId?: number; }) => {
    return api.post<{
      error: string; success: boolean; review: any 
}>('/api/reviews', data);
  },
};
