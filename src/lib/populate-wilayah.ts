...
const populateWilayah = async () => {
  if (process.env.NODE_ENV === 'development') console.log('Memulai pengambilan data wilayah...');

  try {
    // 1. Ambil Provinces
    if (process.env.NODE_ENV === 'development') console.log('Mengambil data provinces...');
    ...
    if (process.env.NODE_ENV === 'development') console.log(`Berhasil menyimpan ${provinces.length} provinces.`);

    // 2. Ambil Regencies untuk setiap Province
    for (const prov of provinces) {
      if (process.env.NODE_ENV === 'development') console.log(`Mengambil data regencies untuk province ${prov.name}...`);
      ...
      if (!regenciesData.is_success || !Array.isArray(regenciesData.data)) {
        if (process.env.NODE_ENV === 'development') console.warn(`Tidak ada regencies untuk province ${prov.id}, melanjutkan...`);
        continue;
      }
      ...
      if (process.env.NODE_ENV === 'development') console.log(`  - Berhasil menyimpan ${regencies.length} regencies.`);

      // 3. Ambil Districts untuk setiap Regency
      for (const reg of regencies) {
        if (process.env.NODE_ENV === 'development') console.log(`    Mengambil data districts untuk regency ${reg.name}...`);
        ...
        if (!districtsData.is_success || !Array.isArray(districtsData.data)) {
          if (process.env.NODE_ENV === 'development') console.warn(`    Tidak ada districts untuk regency ${reg.id}, melanjutkan...`);
          continue;
        }
        ...
        if (process.env.NODE_ENV === 'development') console.log(`      - Berhasil menyimpan ${districts.length} districts.`);

        // 4. Ambil Villages untuk setiap District
        for (const dist of districts) {
          if (process.env.NODE_ENV === 'development') console.log(`      Mengambil data villages untuk district ${dist.name}...`);
          ...
          if (!villagesData.is_success || !Array.isArray(villagesData.data)) {
            if (process.env.NODE_ENV === 'development') console.warn(`      Tidak ada villages untuk district ${dist.id}, melanjutkan...`);
            continue;
          }
          ...
          if (process.env.NODE_ENV === 'development') console.log(`        - Berhasil menyimpan ${villages.length} villages.`);
        }
      }
    }

    if (process.env.NODE_ENV === 'development') console.log('Selesai mengisi data wilayah.');
  } catch (err: any) {
