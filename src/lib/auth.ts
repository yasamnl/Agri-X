...
export function verifyAccessToken(token: string): JwtPayload | null {
  try {
    if (!token) {
      if (process.env.NODE_ENV === 'development') console.log('[verifyAccessToken] No token provided');
      return null;
    }
    ...
    if (parts.length !== 3) {
      if (process.env.NODE_ENV === 'development') console.log('[verifyAccessToken] Invalid token format:', token.substring(0, 50) + '...');
      return null;
    }
    ...
    if (process.env.NODE_ENV === 'development') console.log('[verifyAccessToken] Decoded token:', {
      sub: decoded.sub,
      role: decoded.role,
      exp: decoded.exp,
    });
    ...
    if (decoded.exp < now) {
      if (process.env.NODE_ENV === 'development') console.log('[verifyAccessToken] Token expired. Exp:', decoded.exp, 'Now:', now);
      return null;
    }
