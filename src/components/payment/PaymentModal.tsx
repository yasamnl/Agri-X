  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (!(window as any).snap) {
        ...
        script.async = true;
        script.onload = () => {
          if (process.env.NODE_ENV === 'development') console.log('✅ Snap.js loaded');
          setSnapLoaded(true);
          setIsLoading(false);
        };
        script.onerror = () => {
    ...
    if (isOpen && snapLoaded && (window as any).snap && snapToken) {
      ...
      (window as any).snap.pay(snapToken, {
        ...
        onSuccess: (result: any) => {
          if (process.env.NODE_ENV === 'development') console.log('✅ Payment success:', result);
          onClose({
            eventType: 'success',
        ...
        onPending: (result: any) => {
          if (process.env.NODE_ENV === 'development') console.log('⏳ Payment pending:', result);
          onClose({
            eventType: 'pending',
            paymentType: result.payment_type,
        ...
        onClose: () => {
          if (process.env.NODE_ENV === 'development') console.log('❌ Modal closed');
          onClose({ eventType: 'modal_closed' });
        },
      });
    }
