...
import { formatCurrency } from '@/lib/account-helpers';
...
export function SellerDashboard() {
  ...
  const { data, isLoading, error, refetch, lastUpdated } = useSellerData({
    enabled: true,
    cacheDuration: 30000, // 30s cache
    throttleDuration: 10000, // 10s throttle
  });
  ...
  // ✅ Loading state (hanya saat pertama kali load)
  ...
  // ✅ Error state (hanya jika semua data gagal)
  ...
  const totalOrders = chart.reduce((sum, d) => sum + d.orders, 0);
            ...
            <div>
              ...
              <p className="text-lg font-bold text-text-primary">
                {chartMode === 'revenue'
                  ...
                  : `${Math.round(totalOrders / 7)} pesanan`}
              </p>
            </div>
          </div>
        </div>
