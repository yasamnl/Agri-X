...
interface ProductCardProps {
  ...
  referralCode?: string | null; // Kode referral dari halaman (opsional)
}
...
export function ProductCard({
  ...
  // Hitung sisa kuota Pre-Order
  const remainingQuota = isPreOrder
    ? (product.po_quota ?? 999999) - (product.po_sold || 0)
    : null;

  //  * Handle share produk via Web Share API atau copy link
  //  * Hanya tersedia jika referralCode ada (user adalah affiliate)
  const handleShare = async (e: React.MouseEvent) => {
    ...
    // Coba gunakan Web Share API (mobile)
    if (navigator.share) {
      try {
  ...
  //  * Validasi pre-order: cek kuota dan harvest date
  const handleAddToCart = async (e: React.MouseEvent) => {
    e.stopPropagation();

    // Validasi: Cek kuota Pre-Order
    ...
    // Validasi: Cek tanggal panen Pre-Order
    ...
    try {
      // Tambahkan ke keranjang (quantity = 1)
      await addToCart(product.id, 1);

      // Toast sukses dengan tombol "Lihat Keranjang"
      toast.success(
        <div className="flex items-center justify-between gap-2">
      ...
      // Error handling dengan berbagai kasus
      ...
      if (errorMessage.includes("Kuota") || errorMessage.includes("quota")) {
        ...
        errorMessage.includes("panen") ||
        errorMessage.includes("harvest")
      ) {
        toast.error(
          <div className="flex items-center justify-between gap-2">
  ...
  // Normalisasi image URL dengan fallback
  const imageUrl = (() => {
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square bg-linear-to-br from-secondary/10 to-primary/10 rounded-xl mb-2 overflow-hidden group-hover:scale-105 transition-transform duration-300">
        ...
        {/* Quick Add Button - Muncul saat hover */}
        <button
          onClick={handleAddToCart}
          disabled={
            isAdding ||
            product.status === "sold_out" ||
            (isPreOrder && remainingQuota !== null && remainingQuota <= 0)
          }
        ...
        {/* Tombol Share - Muncul hanya jika referralCode ada (artinya user adalah affiliate) */}
        {referralCode && (
          <button
            onClick={handleShare}
      ...
      <div className="space-y-1 px-1">
        ...
        <div className="flex items-center gap-1 flex-wrap min-h-5">
          <span
            className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${getStatusColor()}`}
          >
            {getStatusText()}
          </span>
        ...
        {/* Pre-Order Quota - Menampilkan sisa kuota */}
          ...
          >
            <AlertTriangle
              className={`w-3 h-3 shrink-0 ${
                remainingQuota <= 10 ? "text-orange-500" : ""
              }`}
            />
