...
import { Star, CalendarDays, AlertTriangle, Plus } from 'lucide-react';
...
export function ProductCard({ product }: { product: Product }) {
  ...
  // Normalize status to a safe string (convert null/undefined and unify hyphen/underscore)
  ...
  // ✅ ✅ ✅ FIX: Gunakan normalizeImageUrl untuk handle URL dengan benar
  const imageUrl = (() => {
    if (imageError) return null;
    const rawUrl = product.image_path || product.image;
    if (!rawUrl) return null;
    
    // ✅ Normalize: convert full URL localhost → relative path
